...
export function LedgerSelector({ ledgers, currentLedgerId, onSelect, onAdd, onDelete, onRename }: LedgerSelectorProps) {
  const { t } = useLanguage();
  const [isOpen, setIsOpen] = useState(false);
  const [isLedgerListOpen, setIsLedgerListOpen] = useState(false);
  const menuRef = useRef<HTMLDivElement>(null);
  const ledgerListRef = useRef<HTMLDivElement>(null);
  const currentLedger = ledgers.find(t => t.id === currentLedgerId);
  ...
  useEffect(() => {
    const handleClickOutside = (event: MouseEvent) => {
      ...
      if (ledgerListRef.current && !ledgerListRef.current.contains(event.target as Node)) {
  ...
  return (
    <div className="flex items-center gap-1">
      {/* Custom Ledger Selector Dropdown */}
      <div className="relative" ref={ledgerListRef}>
        <button
        ...
        >
          <span className="truncate">{currentLedger?.name}</span>
          <ChevronDown className={cn("w-4 h-4 text-gray-500 transition-transform duration-200", isLedgerListOpen && "rotate-180")} />
        </button>
                ...
                {ledgers.map(t => (
                  <button
                    ...
                    className={cn(
                      ...
                      t.id === currentLedgerId
                        ? "bg-blue-50 dark:bg-blue-900/30 text-blue-600 dark:text-blue-400"
                        : "text-gray-700 dark:text-gray-200 hover:bg-gray-50 dark:hover:bg-gray-700/50"
                    )}
                  >
                    <span className="truncate">{t.name}</span>
                    {t.id === currentLedgerId && <Check size={16} />}
                  </button>
                ))}
              </div>
            </motion.div>
...
export function PeopleWallet({ ledger, onAddPerson, onEditPerson, onRemovePerson, onAddExchange, onRemoveExchange }: PeopleWalletProps) {
  const { t } = useLanguage();
    ...
    <div className="space-y-4">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 transition-colors duration-200">
        ...
        <form onSubmit={handleAddPerson} className="flex gap-2 mb-4">
          <input 
            ...
            placeholder={t('ledger_name_placeholder')}
        ...
        <ul className="space-y-1">
          ...
          {ledger.users.length === 0 && <li className="text-gray-400 dark:text-gray-500 text-xs italic px-3">{t('ledger_no_people')}</li>}
        </ul>
      </div>
      ...
      <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-sm border border-gray-100 dark:border-gray-700 p-6 transition-colors duration-200">
        ...
        <form onSubmit={handleAddExchange} className="space-y-3 mb-6">
          <div className="grid grid-cols-1 gap-3">
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1">{t('ledger_cur_placeholder')}</label>
              <input 
                type="text" 
            ...
            <div className="grid grid-cols-2 gap-3">
              <div className="space-y-1">
                <label className="text-[10px] font-bold text-gray-400 uppercase tracking-wider ml-1">{t('ledger_foreign_placeholder')}</label>
                <input 
                  type="text" 
                  inputMode="decimal"
                  pattern="[0-9]*\.?[0-9]*"
        ...
        <div className="space-y-3">
          ...
          {ledger.exchanges.length === 0 && <p className="text-gray-400 dark:text-gray-500 text-xs italic px-3">{t('ledger_no_exchanges')}</p>}
        </div>
      </div>
    </div>
  );
